...
| Anything else — including bare template names ("用 academic_defense 模板"), style descriptions ("咨询风格" / "极简风"), brand mentions ("某企业品牌风格"), vague intent ("想用个模板"), or silence | Skip Step 3, free design |
...
> Bare template names (for example, "academic_defense") do NOT trigger Step 3 even if a folder by that name exists in the library. The user must give a path. AI must not "helpfully" resolve a name to a path.

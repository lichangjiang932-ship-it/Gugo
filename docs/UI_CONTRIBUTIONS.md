...
注册结果按 `order`、再按稳定 key 排序。宿主核心 route、settings section 和 workbench tab 是保留目标，扩展不能覆盖；不同插件也不能声明相同目标。snapshot 变更通知使用固定批次并逐 observer 隔离异常；监听器在通知期间退订、新增或抛错都不能改变当前批次，也不能让已提交注册失去 disposer 或反向改变卸载结果。`getUiPlugin()` / `listUiContributions()` 的查询标识只接受真实字符串，不执行对象 coercion callback；`listUiPlugins()` 及其生命周期快照均冻结。

...
注册结果按 `order`、再按稳定 key 排序。宿主核心 route、settings section 和 workbench tab 是保留目标，扩展不能覆盖；不同插件也不能声明相同目标。snapshot 变更通知使用固定批次并逐 observer 隔离异常；监听器在通知期间退订、新增或同步抛错都不能改变当前批次，也不能让已提交注册失去 disposer 或反向改变卸载结果。监听器返回的真正 Promise 仅通过内建 `Promise.prototype.then` 消化 rejection；自定义 thenable 不会被 assimilate 或执行 `then()`。`getUiPlugin()` / `listUiContributions()` 的查询标识只接受真实字符串，不执行对象 coercion callback；`listUiPlugins()` 及其生命周期快照均冻结。

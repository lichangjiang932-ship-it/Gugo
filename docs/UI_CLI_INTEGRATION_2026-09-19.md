...
### Windows worker 根因收口与停止逐次发布

v0.11.61 本地冻结全量最终为 1026 文件、9060 测试，9051 pass / 9 skip / 0 fail。main CI `35428463792` 完整通过，但 Release `35428463915` 的 Windows job 两次均在 worker 原有 30 秒 READY 等待中失败；用户命令尚未启动，打包/发布被门禁阻止，没有生成新的公开 Release。没有第三次机械重跑。

新增只保留白名单阶段及耗时的诊断后，短流程 `35433488405` 在不执行用户命令的情况下复现：首例卡在未限定模块的 `Add-Type`，30 秒超时；另两例该调用也各耗约 24.5/22.6 秒。独立 WinPS 5.1 影子模块控制还证实了该未限定依赖的信任边界问题。

修复只把 worker 的 Utility 依赖固定到当前系统 PowerShell `$PSHOME`，直接调用导出的 CmdletInfo，不经过环境模块搜索；缺失内置依赖拒绝执行，不增加超时、不改变用户命令环境或权限。native Job Object/进程身份实现按职责独立，生成字节逐字节一致。诊断先输出阶段证据，再有界等待自身进程关闭；未确认清理不能当成功。

此前已启动的验证 `35435640465` 随后通过：三个 worker 3027/234/226ms READY，70 项关联回归 68 pass / 2 skip / 0 fail。影子模块、缺依赖不回退、实际 bind/kill、取消、任务 deadline 及状态证据有行为回归。完整证据及边界见 [WINDOWS_PROCESS_GUARD.md](WINDOWS_PROCESS_GUARD.md)。

用户已要求停止“边修边推、逐次升版”。此后不再增加版本号、不推送、不创建标签或远程构建；先冻结当前源码，统一完成本地 CLI/Web/打包验收，再确认最终版本和一次性交付。已有版本标签不擅自移动。最后一次本地全量采用独立日志 `full-tests-local-final.log`，结果未完成前不能沿用上一版计数声称通过。

### 本地统一验收完成（未发布）

源码基于 `001be6a`，加本地桌面包进程模块完整性校验。该校验现在逐一检查 worker/runtime/native source/启动诊断模块，并通过真实 ASAR 的逐个缺文件负例；没有放宽打包规则或执行权限。最终回归期间没有修改应用源码。

- 全量：**1029 文件、9074 测试，9065 pass / 9 skip / 0 fail / 0 cancelled**，进程退出 0。925 个普通文件的十批及 104 个隔离 UI 文件均完整结束，日志为 `full-tests-local-final.log`。
- 全仓 lint、typecheck 及 14 个反向 fixture、依赖清单、复杂度和 debt 13/13 通过；离线评测 61/61，桌面打包契约 25/25 通过。402 个生产包许可通过。生产审计仍采用上文两项既有、限版本且有到期日的例外，不是“零漏洞”。
- 网页生产构建通过；新的网页归档在全新临时目录重新解包、安装生产依赖，CLI version/help 与隔离服务健康检查均通过，未读取真实模型配置或数据库。
- Windows NSIS 安装包、blockmap 与 latest.yml 已在独立 `local-final-desktop` 目录构建。真实打包后的 Electron 43.3.0 / Node 24.18.1 后端健康检查通过；从 ASAR 导入进程模块后实际执行自建固定命令成功，已取消命令拒绝启动，worker 仅创建一次。
- 安装器与应用的 Authenticode 实测均符合明确 unsigned 策略；四项本地产物 SHA-256、latest.yml 的版本/路径/字节数/SHA-512、六个关键进程模块与源码的字节一致性均通过。记录为 `local-final-candidate-verified.json`，明确标注 `qaOnly: true`、`published: false`。

核对期间保留了一次临时校验脚本失败：它给 Windows 上的 `@electron/asar` 3 传了 POSIX 分隔符，错误报告模块缺失。实际归档列表含该文件，使用系统路径分隔符提取后字节完全一致，且打包程序的真实进程执行已经通过。只修正临时校验器，未重打包、未改生产代码，失败记录 `local-final-candidate-verification.json` 与随后成功记录分别保留。

以上是本地验收包，不是 GitHub Release；没有对这些本地文件声称远程来源证明。用户要求停止之后没有新增提交、推送、标签或版本。包内保留原有 0.11.61 元数据仅用于验收，最终对外版本待用户确认，不擅自重设已有 v0.11.57 标签。

本轮仍不声明真实收费供应商全矩阵、所有实体终端/键盘组合或安装向导交互已测。没有在用户当前安装目录运行覆盖安装，也没有用“默认应用打开”未经实际操作的路径冒充完整验收。已完成的真实浏览器、CLI/网页接续与本机模型证据保持上文范围。

### 用户确认沿用 0.11.61 统一交付

本地统一验收完成后，用户明确选择“按照现有版本来就行”并继续交付，因此不回退 0.11.57，也不再递增版本。只读核验确认远程 v0.11.61 仍指向旧 `6e99b20dd1bcfe853e7ef58719cea9267fbf2bef`，该版本没有已发布或草稿 Release。

交付只将这一尚未发布 Release 的标签对齐最终验收提交，使用精确旧 ref 的 lease 与分支 fast-forward 约束；保留原提交、失败工作流及其他标签。临时 Windows 诊断 workflow 改为仅手动触发，避免最终推送重复启动诊断。原有完整 Release CI、签名策略、五资产、校验和及来源证明门禁没有改动。此追加及其定向发布回归不改变已经冻结并完成全量测试的应用源码；发布成功必须另以实际工作流和下载核验为准。

提交前发布/发现/来源证明/策略/Web 隔离/桌面打包定向回归 **127/127**，定向 lint 通过；独立只读复核的发布/发现/来源证明 **71/71** 通过（与前一组有重叠，不相加）。远程旧 v0.11.61 Release 运行已结束，没有同版本草稿或已发布记录。推送必须使用最终完整 SHA，不使用其他工作树中的旧本地 main 作为来源；任何 lease、快进或保护规则拒绝都应整体停止，不改用裸 force 或拆步绕过。

...
- Node.js `^20.19.0`、`^22.13.0` 或 `>=24.0.0`。使用本机 Edge/Chrome 自动化遵循同一版本范围；Node.js 22 LTS 为推荐版本。
- npm（随 Node.js 安装）。
- Git，用于克隆和升级源码。
...
3. 应用会直接采用服务启动目录（或显式配置的 `WORKSPACE_ROOT`）作为默认工作区，不显示首次配置向导。需要更换目录、收紧文件/Shell/Git 能力或调整审批模式时，再进入“权限中心”；远程请求不能修改本机 owner 的工作区配置，部署策略锁定的开关仍只能由管理员修改。
...
npm run lint         # ESLint
npm test             # 全量自动化测试
npm run i18n:check   # 中英文键对称性与硬编码检查
...
已同步的 Session 目录、工作目录和消息正文以服务端数据库为权威来源；浏览器 IndexedDB 仍保存尚未提交的本地草稿、当前活跃 Turn 的轻量恢复状态及界面偏好。进入“设置 → 数据 & 导出”导出浏览器数据，并将导出文件与服务端备份一起保存。
...
v0.11.50 首次启动会把数据库升级到 schema v114。v113 创建带全局单调游标与持久 stream epoch 的 Agent Event outbox；初始截断游标为 0。v114 新增稳定 durable subscription、租约/fencing、持久 ACK、有界重试/DLQ 和安全裁剪所需的状态表。v113 及更旧的二进制会拒绝打开 v114 数据库；不要把旧程序直接指向已经升级的数据目录。若必须回滚，请停止服务，恢复升级前完整备份的 `APP_DATA_DIR`（以及匹配的凭据密钥），再启动旧版本。本版本不提供 v114 的原地降级迁移。

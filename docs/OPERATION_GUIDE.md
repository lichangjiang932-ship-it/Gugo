...
v0.11.50 首次启动会把数据库升级到 schema v113，并创建带全局单调游标与持久 stream epoch 的 Agent Event outbox；初始截断游标为 0。v112 及更旧的二进制会拒绝打开 v113 数据库；不要把旧程序直接指向已经升级的数据目录。若必须回滚，请停止服务，恢复升级前完整备份的 `APP_DATA_DIR`（以及匹配的凭据密钥），再启动旧版本。本版本不提供 v113 的原地降级迁移。

...
|---|---|---|
| `server/services/TurnEngine.js` | Turn execution, recovery, and cancellation shell; persistence construction is delegated to `turnEnginePersistenceAssembly.js`, while durable start staging and singleton host assembly are already extracted | Finish making persistence dependencies required ports, then continue reducing it to an adapter/context execution shell |

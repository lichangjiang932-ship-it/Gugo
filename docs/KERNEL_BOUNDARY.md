...
|---|---|---|---|
| `server/services/jobRuntime.js` | Job command facade; scheduler ownership and shutdown are delegated to `jobRuntimeLoopHost.js`; the former monolithic tick is split across wake processing, lease claim, preparation/recovery, terminal completion, and step-execution runtimes; default policy composition, planning, crash recovery, and owner-aware event delivery remain behind focused modules | Keep extracting independent capability providers without moving loop lifecycle, tick transitions, default capability composition, planning details, recovery persistence, or event routing back into the facade | `DEBT-ARCH-002` |

...
|---|---|---|---|
| `server/services/jobRuntime.js` | Job policy, scheduling, and loop hosting; default planning composition is delegated to `jobRuntimeDefaultPlanner.js`, crash recovery to `jobRuntimeRecovery.js`, and owner-aware event delivery plus terminal-evicted owner caching to `jobRuntimeEventHub.js` | Keep extracting independent services and capability providers without moving planning composition, recovery persistence, or event routing back into the facade | `DEBT-ARCH-002` |

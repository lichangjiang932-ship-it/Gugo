...
本地模式通过回环地址打开应用后，会自动把服务启动目录（或显式配置的 `WORKSPACE_ROOT`）授予本地 owner、标记为可信工作区并作为新对话默认目录，不显示首次配置向导。文件、Shell、Git 能力按部署上限启用，审批模式默认保持 `normal`；远程请求不能触发这项自动初始化。

需要更换目录、收紧能力或调整审批模式时，可随时进入「权限中心」。`.env`、`.gugo/runtime.json`、`APP_CONFIG_PATH` 或系统环境变量中显式设置的同名开关仍按分层优先级生效，并在界面中显示为由部署策略管理；界面不能覆盖它们。自动初始化只授权默认工作区，不会开放整台电脑，也不把 Shell 变成安全沙箱。

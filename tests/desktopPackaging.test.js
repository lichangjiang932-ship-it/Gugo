...
test('desktop package keeps Electron isolated from renderer content', () => {
  ...
  const mainWindowSecurity = read('desktop/mainWindowSecurity.js')
  ...
  assert.match(mainWindowSecurity, /setWindowOpenHandler/)
  assert.match(mainWindowSecurity, /will-navigate/)
  assert.match(mainWindowSecurity, /will-redirect/)
  assert.match(mainWindowSecurity, /will-attach-webview/)
  assert.match(mainWindowSecurity, /setPermissionRequestHandler/)
  assert.match(mainWindowSecurity, /setPermissionCheckHandler/)
...
test('NSIS package includes server runtime dependencies and updater metadata', () => {
  ...
  const main = read('desktop/main.js')
  const mainWindowSecurity = read('desktop/mainWindowSecurity.js')
  const updateSetup = read('desktop/updateSetup.js')
  ...
  assert.equal((`${main}\n${mainWindowSecurity}`.match(/icon:\s*appIconPath/g) || []).length, 2)
  ...
  assert.match(updateSetup, /createDesktopUpdateRuntime\(\{/)
  assert.match(main, /updateBaseUrl:\s*process\.env\.GUGO_UPDATE_BASE_URL/)
  assert.match(main, /desktopUpdateRuntime\.checkForUpdates\(\)/)
  assert.match(updateSetup, /autoUpdater\.autoDownload\s*=\s*false/)
  assert.doesNotMatch(updateSetup, /autoUpdater\.autoDownload\s*=\s*true/)
  assert.match(updateSetup, /autoUpdater\.autoInstallOnAppQuit\s*=\s*false/)
  assert.doesNotMatch(updateSetup, /autoUpdater\.autoInstallOnAppQuit\s*=\s*true/)
  assert.equal((main.match(/desktopUpdateRuntime\.checkForUpdates\(\)/g) || []).length, 1)
  assert.doesNotMatch(`${main}\n${updateSetup}`, /UPDATE_INTERVAL_MS|setTimeout\(check|setInterval\(check/)
  assert.match(updateSetup, /Local-first default:[\s\S]*explicitly chooses "check and download"/)
  assert.doesNotMatch(updateSetup, /autoUpdater\.on\('download-progress'/)
  assert.match(updateSetup, /autoUpdater\.allowPrerelease\s*=\s*false/)
  assert.match(updateSetup, /autoUpdater\.allowDowngrade\s*=\s*false/)
  assert.match(packageJson.scripts['desktop:check'], /desktop\/updateRuntime\.js/)
  assert.match(packageJson.scripts['desktop:check'], /tests\/desktopUpdateRuntime\.test\.js/)
...
test('version tags publish a Windows installer and updater metadata through GitHub Releases', () => {
  ...
  assert.match(workflow, /- run:\s*npm ci[\s\S]*- name:\s*Package signed Windows desktop installer[\s\S]*npm run desktop:package:signed/)
  assert.match(workflow, /- name:\s*Package explicitly unsigned Windows desktop installer[\s\S]*windows_signing == 'unsigned'[\s\S]*npm run desktop:package:unsigned/)

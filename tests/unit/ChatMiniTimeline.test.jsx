...
test('timeline previews, selects, highlights, and appends conversation turns', async () => {
  ...
  try {
    ...
    assert.ok(activeStroke.classList.contains('h-px'))
    assert.ok(activeStroke.classList.contains('w-4'))
    assert.ok(idleStroke.classList.contains('w-2.5'))
    assert.ok(activeStroke.classList.contains('bg-ink/55'))
    assert.ok(idleStroke.classList.contains('bg-ink/20'))
...
test('deep timelines keep a fixed button bound with accessible earlier and later navigation', async () => {
  ...
  try {
    ...
    assert.ok(earlier.querySelector('svg[aria-hidden="true"]'))
    assert.ok(later.querySelector('svg[aria-hidden="true"]'))

    earlier.focus()
    assert.equal(dom.window.document.activeElement, earlier)
    const move = new dom.window.KeyboardEvent('keydown', { key: 'ArrowDown', bubbles: true, cancelable: true })
    await act(async () => earlier.dispatchEvent(move))
    assert.equal(move.defaultPrevented, true)
    assert.equal(dom.window.document.activeElement, markers[0])
    assert.deepEqual(selected, [], 'arrow navigation moves focus without selecting a turn')
    await act(async () => markers[0].dispatchEvent(new dom.window.KeyboardEvent('keydown', { key: 'End', bubbles: true })))
    assert.equal(dom.window.document.activeElement, later)

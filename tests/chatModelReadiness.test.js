...
test('chat send guards model readiness before clearing drafts or creating session state', () => {
  ...
  assert.match(acceptedDraft, /if \(attachmentsAccepted\) setAttachments\?\.\(nextAttachments\)/)
  ...
  assert.match(page, /const showModelUnavailable = \(readiness = modelReadiness\) => \{[\s\S]*setShowModelPicker\(true\)[\s\S]*\}/)
  assert.doesNotMatch(
    page.match(/const showModelUnavailable = \(readiness = modelReadiness\) => \{[\s\S]*?\n {2}\}/)?.[0] || '',
    /setWorkbenchMessage/,
  )
})

...
test('composer keeps restrained elevation while tool rows stay grounded', () => {
  ...
  assert.doesNotMatch(toolCard, /border-radius|box-shadow/)
  assert.match(toolCard, /padding:\s*0\.22rem 0/)
  assert.match(toolCard, /grid-template-columns:\s*1\.15rem minmax\(0, 1fr\)/)
  assert.match(toolCard, /gap:\s*0\.4rem/)
  const marker = cssRule('.chat-tool-step-marker')
  assert.match(marker, /width:\s*1rem/)
  assert.match(marker, /height:\s*1rem/)
  assert.match(marker, /border:\s*0/)
  assert.match(cssRule('.chat-run-timeline .chat-tool-step'), /display:\s*grid/)
  assert.doesNotMatch(cssRule('.chat-run-timeline .chat-tool-step-marker'), /clip:|width:\s*1px|position:\s*absolute/)

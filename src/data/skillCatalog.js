...
- Default to offline-safe output: no CDN, remote font, tracker, iframe, fetch, or invented image URL. Use CSS, inline SVG, or user-provided assets. When create_html_app exposes an assets field, declare every existing local image/audio/video there and reference each exact id as gugo-asset://<id>; those files are inputs, so never call generate_image unless the user separately asks for a new image. Use remote assets only when the user explicitly permits them.
...
for (const skill of SKILLS) {
  ...
  const visualGuardrails = skill.id === 'ppt' ? `\n\n${PPT_VISUAL_GUARDRAILS}` : ''
  skill.systemPrompt = `${SHARED_SKILL_GUARDRAILS}\n\n${focusedPrompt}${visualGuardrails}`
}

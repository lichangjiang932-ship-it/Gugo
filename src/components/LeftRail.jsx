...
export default function LeftRail() {
  ...
  return <>
    ...
    <aside role="navigation" aria-label={`Gugo · ${t('nav.chat')}`} data-collapsed={collapsed ? 'true' : 'false'} className={`flex h-full shrink-0 flex-col border-r border-ink/[0.07] bg-paper transition-[width] duration-200 ${collapsed ? 'w-[60px] px-2 py-2.5' : 'w-[264px] px-2.5 py-2.5'} ${narrowViewport && mobileExpanded ? 'fixed inset-y-0 left-0 z-50 shadow-2xl' : ''}`}>
      <header className={`flex h-10 items-center ${collapsed ? 'justify-center' : 'gap-2 px-1'}`}>
        {!collapsed && <button type="button" onClick={() => { controller.closeSessionMenu(); closeMobileRail(); navigate('/chat') }} aria-label="Gugo" className="flex min-w-0 flex-1 items-center gap-2.5 rounded-lg text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ember/30"><BrandMark className="h-7 w-7 shrink-0 text-ember" /><span className="min-w-0"><span className="block truncate text-[14px] font-semibold leading-4 text-ink">Gugo</span><span className="block truncate text-[10px] leading-4 text-ink-fade">{t('nav.chat')}</span></span></button>}
        <button type="button" onClick={() => setRailCollapsed(!collapsed)} title={collapsed ? t('chatMessages.showSidebar') : t('chatMessages.hideSidebar')} aria-label={collapsed ? t('chatMessages.showSidebar') : t('chatMessages.hideSidebar')} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-ink-fade transition-colors hover:bg-ink/[0.05] hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ember/30">{collapsed ? <PanelLeftOpen className="h-4 w-4" /> : <PanelLeftClose className="h-4 w-4" />}</button>
      </header>

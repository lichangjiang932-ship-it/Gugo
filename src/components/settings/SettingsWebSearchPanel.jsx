...
function Field({ label, value, onChange, placeholder = '', multiline = false, type = 'text' }) {
  const className = 'min-w-0 w-full rounded-lg border border-[#e8e6e0] bg-white px-3.5 py-2.5 text-sm text-[#1c1c1a] outline-none transition-colors placeholder:text-[#9a9890] hover:border-[#d8d6cf] focus:border-[#4070ec] focus:ring-2 focus:ring-[#4070ec]/10'
  return <label className="flex min-w-0 flex-col gap-1.5 text-xs text-[#66645e]"><span>{label}</span>{multiline
...
export default function SettingsWebSearchPanel({ t }) {
  ...
  return <section className="web-search-settings flex min-w-0 max-w-full flex-col gap-5 text-[#1c1c1a]">
    <div><span className="font-mono text-[9px] tracking-[0.22em] text-[#77746d]">WEB SEARCH</span><h1 className="mt-1 text-[22px] font-semibold leading-tight text-[#1c1c1a]">{t('webSearch.title')}</h1><p className="mt-1 text-sm text-[#66645e]">{t('webSearch.subtitle')}</p></div>
    <div className="flex min-h-[60px] min-w-0 items-center gap-3 rounded-xl bg-[#ecf6ef] px-4 py-3"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#d8efe1]"><ShieldCheck className="h-4 w-4 text-[#328068]" /></span><p className="min-w-0 flex-1 text-xs leading-5 text-[#66645e]">{t('webSearch.security')}</p><Toggle enabled={enabled} onChange={setEnabled} label={t('webSearch.enabled')} /></div>
    <div className="flex min-w-0 flex-col gap-4 rounded-xl border border-[#e8e6e0] bg-white p-4 sm:p-[18px]">
      <div className="flex min-w-0 flex-col items-stretch justify-between gap-3 sm:flex-row sm:items-start sm:gap-4"><div className="min-w-0"><h2 className="text-base font-medium text-[#1c1c1a]">{t('webSearch.provider')}</h2><p className="mt-1 text-xs leading-5 text-[#66645e]">{t('webSearch.fallbackHint')}</p></div><button type="button" onClick={addConnection} disabled={connections.length >= 8 || Boolean(working)} className="inline-flex h-9 shrink-0 items-center justify-center gap-2 rounded-lg border border-[#e8e6e0] bg-white px-3.5 text-xs text-[#1c1c1a] transition-colors hover:bg-[#f5f5f0] disabled:opacity-50"><Plus className="h-3.5 w-3.5" />{t('webSearch.addApi')}</button></div>
      <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(190px,0.72fr)_minmax(0,1.7fr)]">
        <div className="flex min-w-0 flex-col gap-2.5" data-testid="web-search-connections">
          {connections.map((item, index) => {
            ...
            return <div key={item.id} className={`min-w-0 rounded-xl border py-3 pr-3 transition-colors ${active ? 'border-[#e8e6e0] border-l-4 border-l-[#4070ec] bg-[#f5f5f0] pl-[9px]' : 'border-[#e8e6e0] bg-white pl-3 hover:bg-[#fafaf7]'}`}>
        ...
        {selected ? <div className="flex min-w-0 flex-col gap-4 rounded-xl border border-[#e8e6e0] bg-white p-4" data-testid="web-search-connection-editor">
          <div className="flex min-w-0 flex-col items-stretch justify-between gap-3 sm:flex-row sm:items-center"><div className="flex min-w-0 items-center gap-2.5"><BrandMark item={meta} size="lg" /><div className="min-w-0"><h3 className="truncate text-sm font-medium text-[#1c1c1a]">{meta.label}</h3><p className="mt-1 text-[10px] text-[#77746d]">{t('webSearch.apiSettings')} · {t('webSearch.priority', { index: selectedIndex + 1 })}</p></div></div><div className="flex items-center justify-between gap-2.5 sm:justify-start"><span className="text-xs text-[#66645e]">{t('webSearch.useThisApi')}</span><Toggle enabled={selected.enabled} onChange={(value) => replaceConnection(selected.id, (item) => ({ ...item, enabled: value }))} label={t('webSearch.useThisApi')} /></div></div>
          <div className="grid min-w-0 grid-cols-2 gap-2.5 lg:grid-cols-3" data-testid="web-search-template-grid">{WEB_SEARCH_PROVIDERS.map((item) => { const chosen = selected.provider === item.id; return <button key={item.id} type="button" data-provider={item.id} onClick={() => replaceConnection(selected.id, (current) => item.id === current.provider ? current : { ...current, provider: item.id, config: {}, apiKey: '', apiKeyPresent: false })} className={`relative min-w-0 rounded-xl border bg-white p-2.5 text-left transition-colors ${chosen ? 'border-[#4070ec]' : 'border-[#e8e6e0] hover:border-[#d8d6cf] hover:bg-[#f5f5f0]'}`}>{chosen && <Check className="absolute right-2 top-2 h-3 w-3 text-[#4070ec]" />}<span className="flex min-w-0 flex-col gap-1.5"><BrandMark item={item} /><span className="min-w-0 pr-1"><span className="block min-h-8 whitespace-normal break-words text-[12px] leading-4 text-[#1c1c1a]">{item.label}</span><span className="mt-0.5 block text-[9px] text-[#77746d]">{item.id === 'custom' ? t('webSearch.customBadge') : t('webSearch.presetBadge')}</span></span></span></button> })}</div>
          ...
          {selected.provider === 'custom' ? <div className="grid gap-4 border-t border-dashed border-[#e8e6e0] pt-5">
            ...
            <label className="flex min-w-0 flex-col gap-1.5 text-xs text-[#66645e]"><span>{t('webSearch.method')}</span><select value={selected.config.method || 'POST'} onChange={(event) => updateConfig('method', event.target.value)} className="h-10 min-w-0 w-full rounded-lg border border-[#e8e6e0] bg-white px-3.5 text-sm text-[#1c1c1a] outline-none transition-colors hover:border-[#d8d6cf] focus:border-[#4070ec] focus:ring-2 focus:ring-[#4070ec]/10"><option value="POST">POST</option><option value="GET">GET</option></select></label>

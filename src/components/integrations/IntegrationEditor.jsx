...
function Field({ field, form, onChange, t }) {
  ...
  return <label className="flex flex-col gap-1.5">
    ...
    {field.type === 'select' ? <select value={value} onChange={(event) => onChange(field, event.target.value)} className="h-10 px-3 rounded-md border border-ink-fade/40 bg-paper text-sm outline-none focus:border-ember"><option value="">-</option>{field.options.map((option) => <option key={option} value={option}>{option}</option>)}</select>
      : <input type={field.type === 'password' ? 'password' : field.type === 'url' ? 'url' : 'text'} value={value} placeholder={field.type === 'password' && form.id ? t('integrations.secretPlaceholder') : ''} onFocus={() => { if (field.location === 'secret' && value === SECRET_SENTINEL) onChange(field, '') }} onChange={(event) => onChange(field, event.target.value)} className="h-10 px-3 rounded-md border border-ink-fade/40 bg-paper text-sm outline-none focus:border-ember" />}
  </label>
}
  ...
  return <div className="rounded-md border border-ink-fade/30 p-3 flex flex-col gap-3">
    ...
    {qr?.qrcodeUrl && <div className="flex flex-col sm:flex-row items-center gap-3">
      ...
      <div className="flex flex-col gap-2 min-w-0">
        {state.phase === 'ready' && <span className="inline-flex items-center gap-1.5 self-start h-6 px-2 rounded-full bg-paper-2 text-[11px] text-ink-soft font-mono"><Circle className="w-2 h-2 fill-emerald-500 text-emerald-500" />{t('wechat.qr.expiresIn', { seconds: state.secondsLeft })}</span>}
        ...
        {['expired', 'timeout', 'error'].includes(state.phase) && state.errorText && <span className="text-xs text-red-600 leading-relaxed">{state.errorText}</span>}
...
export default function IntegrationEditor({ form, meta, saving, testingId, testMessage, wechatQr, wechatState, onChange, onSave, onTest, onWechatQr, onClose, t }) {
  ...
  return createPortal(<div className="fixed inset-0 z-50 bg-ink/35 flex items-center justify-center p-4">
    <form onSubmit={onSave} className="w-full max-w-lg max-h-[88vh] overflow-y-auto rounded-md border border-ink bg-paper shadow-xl p-5 flex flex-col gap-4">
      ...
      <label className="flex flex-col gap-1.5"><span className="text-xs text-ink-fade">{t('integrations.name')}</span><input value={form.name} onChange={(event) => onChange('name', event.target.value)} className="h-10 px-3 rounded-md border border-ink-fade/40 bg-paper text-sm outline-none focus:border-ember" /></label>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2"><button type="button" onClick={onClose} className="h-9 px-4 rounded-md border border-ink-fade/40 text-sm text-ink-soft hover:bg-paper-2">{t('integrations.cancel')}</button><button disabled={saving || !form.name.trim()} className="h-9 px-4 rounded-md bg-ember text-paper text-sm hover:bg-ember/90 disabled:opacity-50">{form.enabled ? `${t('integrations.save')} & ${t('integrations.enabled')}` : t('integrations.save')}</button></div>
      </div>
    </form>
  </div>, document.body)
}

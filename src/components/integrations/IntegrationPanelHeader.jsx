    ...
    <div>
      ...
      <div className="flex items-center gap-2 mt-0.5">
        ...
        {kind === 'vision_assist' && visionStatus && <span className="relative inline-flex">
          ...
          {visionHintOpen && <span className="absolute z-30 left-0 top-6 w-72 p-2.5 rounded-md border border-ink-fade/40 bg-paper shadow-lg text-xs text-ink-soft leading-relaxed">{t('integrations.visionAssist.badge.hint')}{visionStatus.models?.length ? <span className="block mt-1.5 font-mono text-[10px] text-ink-fade truncate">MODEL_NAMES_VISION = {visionStatus.models.join(', ')}</span> : null}</span>}
        </span>}
      </div>
    </div>

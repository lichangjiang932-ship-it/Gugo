    ...
    <div>
      ...
      <div className="flex items-center gap-2 mt-0.5">
        ...
        {kind === 'vision_assist' && visionStatus && <span className="relative inline-flex">
          <button type="button" onClick={() => onVisionHintChange(!visionHintOpen)} onBlur={() => window.setTimeout(() => onVisionHintChange(false), 120)} className={`inline-flex items-center gap-1 h-5 px-2 rounded-full text-[10px] font-mono cursor-pointer transition-colors ${visionStatus.configured ? 'bg-emerald-50 text-emerald-700 border border-emerald-300 hover:bg-emerald-100' : 'bg-paper-2 text-ink-fade border border-ink-fade/40 hover:bg-paper'}`} aria-label={visionStatus.configured ? t('integrations.visionAssist.badge.active') : t('integrations.visionAssist.badge.inactive')}>
            <span className={`w-1.5 h-1.5 rounded-full ${visionStatus.configured ? 'bg-emerald-500' : 'bg-ink-fade'}`} />{visionStatus.configured ? t('integrations.visionAssist.badge.active') : t('integrations.visionAssist.badge.inactive')}
          </button>
          {visionHintOpen && <span className="absolute z-30 left-0 top-6 w-72 p-2.5 rounded-md border border-ink-fade/40 bg-paper shadow-lg text-[11px] text-ink-soft leading-relaxed">{t('integrations.visionAssist.badge.hint')}{visionStatus.models?.length ? <span className="block mt-1.5 font-mono text-[10px] text-ink-fade truncate">MODEL_NAMES_VISION = {visionStatus.models.join(', ')}</span> : null}</span>}
        </span>}
      </div>
    </div>

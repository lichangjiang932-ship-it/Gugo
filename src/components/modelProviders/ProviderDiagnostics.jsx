...
export default function ProviderDiagnostics({ diagnostics, onClose, t }) {
  ...
  return <div className="border border-ink/15 rounded-md p-3 flex flex-col gap-2">
    ...
    {(diagnostics.steps || []).map((step) => <div key={step.name} className="flex items-start gap-2 text-[11px]">
    ...
    {diagnostics.profile && <div className="text-[11px] text-ink-fade border-t border-ink/10 pt-2 flex flex-wrap gap-x-3 gap-y-1">
    ...
    {diagnostics.error && !diagnostics.steps?.length && <div className="text-[11px] text-rose-700">{diagnostics.error}</div>}
  </div>
}

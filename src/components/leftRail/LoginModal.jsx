...
const inputClass = 'h-9 rounded-md border border-ink/40 bg-paper px-3 text-sm text-ink outline-none focus:border-ember'
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/35 p-4">
    <div className="flex w-full max-w-md flex-col gap-4 rounded-md border border-ink bg-paper p-5 shadow-xl">
      ...
      <form onSubmit={onVerify} className="flex flex-col gap-3">
        ...
        <button disabled={login.loading || !login.email.trim() || (login.mode === 'password' ? !login.password : !login.code.trim())} className="h-9 self-start rounded-md bg-ember px-4 text-sm text-paper transition-colors hover:bg-ember/90 disabled:opacity-50">{t('leftRailLogin.submit')}</button>
      </form>

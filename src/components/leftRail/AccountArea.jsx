...
function MenuButton({ icon: Icon, label, onClick, badge = 0 }) {
  return <button type="button" onClick={onClick} className="flex h-9 w-full items-center gap-2.5 rounded-control px-2.5 text-sm text-ink-soft hover:bg-paper-2"><Icon className="h-4 w-4" /><span className="flex-1 text-left">{label}</span>{badge > 0 && <span data-compact-numeric-badge className="rounded-pill bg-ember px-1.5 text-[10px] text-paper">{badge > 99 ? '99+' : badge}</span>}</button>
}

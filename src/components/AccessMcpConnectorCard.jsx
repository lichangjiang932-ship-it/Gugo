      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {badge}
            {connector.official && <span className="rounded-full border border-blue-200 bg-blue-50 px-2 py-0.5 text-[10px] text-blue-700">{t('access.official')}</span>}
            {ready && <span className="inline-flex items-center gap-1 rounded-full border border-green-200 bg-green-50 px-2 py-0.5 text-[10px] text-green-700"><CheckCircle2 className="h-3 w-3" />{t('access.mcpReady')}</span>}
          </div>
          <p className="mt-1 text-xs leading-5 text-ink-soft">{t(connector.descriptionKey)}</p>
          <p className="mt-1 text-[10px] text-ink-fade">{t('access.publishedBy').replace('{publisher}', connector.publisher)}</p>
          {ready && toolCount > 0 && <p className="mt-2 text-xs text-green-700">{t('access.mcpToolCount').replace('{count}', String(toolCount))}</p>}
          {server && !ready && <p className="mt-2 text-xs text-amber-700">{t('access.mcpNeedsRetry')}</p>}
        </div>
      </div>
      <div className="mt-4 flex items-center justify-end gap-2 border-t border-dashed border-ink-fade/30 pt-3">
        {ready ? (
          <>
            ...
            <button type="button" onClick={() => onRemove(connector, server)} disabled={busy} className="inline-flex h-8 items-center gap-1.5 rounded-md px-2 text-xs text-ink-fade hover:bg-red-50 hover:text-red-600 disabled:opacity-40" aria-label={`${t('access.removeMcp')} ${connector.label}`}><Trash2 className="h-3.5 w-3.5" /></button>
          </>
        ) : (

...
export async function executeToolCall(call, options = {}) {  const { maxRetries = 2, retryDelayMs = 600 } = options
  ...
  if (name && name.startsWith('browser_')) {
    const routes = {
      ...
      browser_tabs: '/api/browser/tabs',
      browser_switch_tab: '/api/browser/switch-tab',
      browser_frames: '/api/browser/frames',
      browser_switch_frame: '/api/browser/switch-frame',
      ...
      browser_upload_file: '/api/browser/upload-file',
      browser_download: '/api/browser/download',

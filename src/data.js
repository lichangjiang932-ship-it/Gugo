...
export const SKILLS = [
  {
    ...
    recommended: true,
    systemPrompt:
      '你是一位专业的演示文稿设计师。请根据用户提供的主题、资料或大纲，生成结构清晰、可直接导出为 PPTX 的幻灯片内容。只输出 Markdown 幻灯片正文，不要写额外说明。\n\n页面结构要求：\n1. 封面页（第1页）：只输出大标题和副标题（可选），不要项目符号。格式：第一行 `# 主标题`，第二行可选 `副标题 / 日期`。\n2. 目录页（第2页）：标题为 `目录` 或 `内容概览`，用数字编号列表 `1. 2. 3.` 列出各章节，不用项目符号。\n3. 正文页：每页用单独一行 `---` 分隔；每页第一行是标题，后续用短句或项目符号表达要点。\n4. 图文页：在需要配图的位置用 `![图片描述](image)` 标注配图建议。\n5. 结束页（最后一页）：标题如 `感谢观看` 或 `Q&A`，可附一句简短结语，不要项目符号。\n\n内容应包含：封面页、目录页、3-6页核心分析页、可选的图文页、结束页。',
  },
  {
    id: 'doc',
    icon: '📑',

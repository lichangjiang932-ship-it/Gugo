...
const FILTER_KEYS = ['all', 'active', 'queued', 'completed', 'failed', 'cancelled']
const STATUS_KEYS = new Set(['queued', 'planning', 'running', 'waiting', 'awaiting_approval', 'completed', 'failed', 'cancel_requested', 'cancelled'])
    ...
    async function refresh() {
      ...
      if (active) {
        ...
        if (jobsResult.status === 'rejected' && jobResult.status === 'rejected') {
          setError(jobsResult.reason?.message || jobResult.reason?.message || t('taskCenter.refreshFailed'))
        }
      }
      refreshing = false
    ...
    <div className="h-screen flex bg-paper overflow-hidden">
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        <header className="px-7 py-5 border-b border-dashed border-ink-fade/40">
          ...
          <h1 className="font-hand text-[28px] text-ink mt-1.5">{t('taskCenter.title')}</h1>
          <form onSubmit={handleCreate} className="mt-4 flex gap-2">
            <input
              value={prompt}
              onChange={(event) => setPrompt(event.target.value)}
              placeholder={t('taskCenter.prompt')}
            ...
            >
              {submitting ? t('taskCenter.creating') : t('taskCenter.start')}
            </button>
          </form>
        ...
        <section className="flex-1 min-h-0 grid grid-cols-[320px_minmax(0,1fr)]">
          <aside className="border-r border-dashed border-ink-fade/40 p-4 overflow-y-auto">
            ...
            {loading ? (
              <p className="text-sm text-ink-fade">{t('taskCenter.loading')}</p>
          ...
          <div className="p-5 overflow-y-auto">
            {!selectedJob ? (
              <div className="h-full flex flex-col items-center justify-center text-center gap-3 text-ink-fade">
                ...
                <p className="text-sm">{t('taskCenter.select')}</p>
              </div>
            ) : (
              <div className="max-w-4xl mx-auto flex flex-col gap-5">
                <div className="rounded-md border border-ink/20 bg-paper-2 p-4">
                  ...
                  <div className="mt-4 grid grid-cols-4 gap-3 text-sm">
                    <div>
                      <p className="text-[11px] text-ink-fade">{t('taskCenter.status')}</p>
                      <p className="text-ink mt-1">{statusLabel(selectedJob.status)}</p>
                    </div>
                    <div>
                      <p className="text-[11px] text-ink-fade">{t('taskCenter.progress')}</p>
                      <p className="text-ink mt-1">{selectedJob.progress}%</p>
                    </div>
                    <div>
                      <p className="text-[11px] text-ink-fade">{t('taskCenter.createdAt')}</p>
                      <p className="text-ink mt-1">{formatTime(selectedJob.createdAt)}</p>
                    </div>
                    <div>
                      <p className="text-[11px] text-ink-fade">{t('taskCenter.updatedAt')}</p>
                  ...
                  {selectedJob.error && (
                    <div className="mt-3 rounded-md border border-dashed border-red-500/50 bg-red-500/5 p-3">
                      <p className="text-[11px] text-red-600">{t('taskCenter.failureReason')}</p>
                  ...
                  {selectedJob.status === 'awaiting_approval' && (
                    <div className="mt-3 rounded-md border border-dashed border-amber-500/50 bg-amber-500/5 p-3 flex items-center gap-3">
                      <p className="text-sm text-ink flex-1">{t('taskCenter.awaitingApproval')}</p>
                      <button
                  ...
                  <section className={`rounded-md border p-4 ${
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-hand text-lg text-ink">
                        {finalIncomplete ? t('taskCenter.deliveryIncomplete') : t('taskCenter.delivery')}
                      </h3>
                    </div>
                    ...
                    {finalEvidence.length > 0 && (
                      <details className="mt-3 border-t border-dashed border-ink-fade/40 pt-3">
                        <summary className="cursor-pointer text-xs font-medium text-ink">
                          {t('taskCenter.evidence', { count: finalEvidence.length })}
                        </summary>
                ...
                <div className="grid grid-cols-[1.2fr_0.8fr] gap-4">
                  <section className="rounded-md border border-ink/20 p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h3 className="font-hand text-lg text-ink">{t('taskCenter.subtasks')}</h3>
                    </div>
                    <div className="flex flex-col gap-2">
                      {(selectedJob.steps || []).map((step) => (
                        <div key={step.id} className="rounded-md border border-dashed border-ink-fade/40 p-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="ml-auto text-xs text-ink-fade">{statusLabel(step.status)}</span>
                          </div>
                          ...
                          {stepAcceptance(step).length > 0 && (
                            <details className="mt-2 text-xs text-ink-fade">
                              <summary className="cursor-pointer">{t('taskCenter.acceptance')}</summary>
                  ...
                  <div className="flex flex-col gap-4">
                    <section className="rounded-md border border-ink/20 p-4">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className="font-hand text-lg text-ink">{t('taskCenter.events')}</h3>
                      </div>
                    ...
                    <section className="rounded-md border border-ink/20 p-4">
                      <div className="flex items-center gap-2 mb-3">
                        <CheckCircle2 className="w-4 h-4 text-ember" />
                        <h3 className="font-hand text-lg text-ink">{t('taskCenter.artifacts')}</h3>
                      </div>
                      {(selectedJob.artifacts || []).length ? (
                            ...
                            return (
                              ...
                              >
                                ...
                                <button
                                  ...
                                  aria-label={t('taskCenter.preview')}
                                >
                        ...
                        <p className="text-xs text-ink-fade">{t('taskCenter.noArtifacts')}</p>
                      )}
                    </section>
                  </div>
                </div>

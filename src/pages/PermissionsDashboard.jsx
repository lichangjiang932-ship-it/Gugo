    ...
    <div className="flex h-screen overflow-hidden bg-paper">
      ...
      <div className="flex-1 overflow-y-auto p-8">
        <div className="mb-6 flex items-end justify-between">
          ...
          <button onClick={controller.runChecks} disabled={controller.checking} className="flex h-9 items-center gap-1.5 rounded-md border border-dashed border-ink-fade/60 px-4 font-semibold text-sm text-ink-soft transition-colors hover:border-ink-fade disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${controller.checking ? 'animate-spin' : ''}`} />{controller.checking ? t('permissionsDashboard.checking') : t('permissionsDashboard.refresh')}</button>
        </div>

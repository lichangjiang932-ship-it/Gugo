...
export default function HooksView() {
  ...
  return <div className="flex h-screen overflow-hidden bg-paper"><LeftRail /><div className="flex min-w-0 flex-1 flex-col">
    <div className="flex items-center gap-3 border-b border-ink/10 px-6 py-4"><Webhook className="h-5 w-5 text-accent-ink" /><div className="flex-1"><div className="text-base font-semibold text-ink">Hooks</div><div className="text-xs text-ink-fade">{t('hooks.subtitle')}</div></div><button type="button" onClick={() => setEditing(createEmptyHook())} className="flex h-8 items-center gap-1 rounded-md bg-accent px-3 text-xs text-accent-contrast hover:bg-accent/90"><Plus className="h-3.5 w-3.5" />{t('hooks.add')}</button></div>

...
export default function McpServerEditor({ controller, t }) {
  ...
  if (!editing) return <div className="flex h-full items-center justify-center px-6 text-center text-sm text-ink-fade">{t('mcp.selectHint')}<br /><span className="text-xs">{t('mcp.transportHint')}</span></div>
  ...
  return (
    <div className="mx-auto max-w-[720px] space-y-4 px-8 py-6">
      ...
      <Field label={t('mcp.name')}><input value={editing.name} onChange={(event) => update({ name: event.target.value })} className="h-9 w-full rounded-md border border-ink/15 bg-paper-2 px-3 text-sm outline-none focus:border-focus" placeholder={t('mcp.namePlaceholder')} /></Field>
      ...
      <div className="flex items-center gap-2 pt-2">
        <button type="button" onClick={controller.save} disabled={controller.saving} className="flex h-8 items-center gap-1 rounded-md bg-accent px-4 text-xs text-accent-contrast hover:bg-accent/90 disabled:opacity-50"><Save className="h-3.5 w-3.5" />{controller.saving ? t('mcp.saving') : t('mcp.save')}</button>
...
function StdioFields({ controller, editing, t, update }) {
  return <><Field label={t('mcp.command')}><input value={editing.command} onChange={(event) => update({ command: event.target.value })} className="h-9 w-full rounded-md border border-ink/15 bg-paper-2 px-3 font-mono text-sm outline-none focus:border-focus" /></Field><Field label={t('mcp.args')}><input value={Array.isArray(editing.args) ? editing.args.join(' ') : editing.args || ''} onChange={(event) => update({ args: event.target.value })} className="h-9 w-full rounded-md border border-ink/15 bg-paper-2 px-3 font-mono text-sm outline-none focus:border-focus" /></Field><Field label={t('mcp.cwd')}><input value={editing.cwd || ''} onChange={(event) => update({ cwd: event.target.value })} className="h-9 w-full rounded-md border border-ink/15 bg-paper-2 px-3 font-mono text-sm outline-none focus:border-focus" placeholder={t('mcp.cwdPlaceholder')} /></Field><Field label={t('mcp.env')} error={controller.fieldErrors.env} hint={t('mcp.keyValueHint')}><textarea rows="4" value={editing.envText || ''} onChange={(event) => { update({ envText: event.target.value }); controller.setFieldErrors((current) => ({ ...current, env: '' })) }} className="w-full rounded-md border border-ink/15 bg-paper-2 px-3 py-2 font-mono text-xs outline-none focus:border-focus" placeholder={'GITHUB_TOKEN=...\nAPI_KEY=...'} spellCheck="false" /></Field></>
}

function RemoteFields({ controller, editing, t, update }) {
  return <><Field label={t('mcp.url')}><input value={editing.url || ''} onChange={(event) => update({ url: event.target.value })} className="h-9 w-full rounded-md border border-ink/15 bg-paper-2 px-3 text-sm outline-none focus:border-focus" placeholder="https://your-mcp.example.com/mcp" /></Field><Field label={t('mcp.headers')} error={controller.fieldErrors.headers} hint={t('mcp.keyValueHint')}><textarea rows="4" value={editing.headersText || ''} onChange={(event) => { update({ headersText: event.target.value }); controller.setFieldErrors((current) => ({ ...current, headers: '' })) }} className="w-full rounded-md border border-ink/15 bg-paper-2 px-3 py-2 font-mono text-xs outline-none focus:border-focus" placeholder={'Authorization=Bearer ...\nX-API-Key=...'} spellCheck="false" /></Field><OAuthFields controller={controller} editing={editing} t={t} update={update} /></>
}

function OAuthFields({ controller, editing, t, update }) {
  const inputClass = 'h-9 w-full rounded-md border border-ink/15 bg-paper px-3 font-mono text-xs outline-none focus:border-focus'
  return (
    <div className="space-y-3 rounded-lg border border-ink/10 bg-paper-2/60 p-4">
      <div className="flex items-center gap-2"><KeyRound className="h-4 w-4 text-accent-ink" /><div className="min-w-0 flex-1"><div className="text-xs font-semibold text-ink">{t('mcp.oauthTitle')}</div><div className="text-[10px] text-ink-fade">{t('mcp.oauthHint')}</div></div>{editing.oauth?.configured && <span className={`rounded-full px-2 py-0.5 text-[10px] ${editing.oauth.connected ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{t(editing.oauth.connected ? 'mcp.oauthConnectedStatus' : 'mcp.oauthExpiredStatus')}</span>}</div>
...
function TestResult({ result, t }) {
  ...
  return <div className="mt-2 space-y-2 rounded-md border border-ink/10 bg-paper-2 p-3 text-xs">{result.tools?.length > 0 && <div><div className="mb-1 font-semibold text-ink">{t('mcp.tools')} ({result.tools.length})</div><ul className="space-y-0.5 text-xs text-ink-soft">{result.tools.map((tool) => <li key={tool.name}><code className="text-accent-ink">{tool.name}</code> · {tool.description}</li>)}</ul></div>}{result.resources?.length > 0 && <div className="font-semibold text-ink">{t('mcp.resources')} ({result.resources.length})</div>}{result.prompts?.length > 0 && <div className="font-semibold text-ink">{t('mcp.prompts')} ({result.prompts.length})</div>}</div>
}

function Field({ label, hint, error, children }) {
  return <div><label className="mb-1.5 block text-xs text-ink-fade">{label}</label>{children}{hint && !error && <p className="mt-1 text-[10px] text-ink-fade">{hint}</p>}{error && <p className="mt-1 text-[10px] text-rose-700">{error}</p>}</div>
}

function Chip({ active, onClick, children }) {
  return <button type="button" onClick={onClick} className={`rounded-md border px-3 py-1 text-xs transition-colors ${active ? 'border-accent bg-accent text-accent-contrast' : 'border-ink/15 bg-paper-2 text-ink-soft hover:border-accent/50'}`}>{children}</button>
}

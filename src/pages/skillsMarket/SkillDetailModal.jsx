...
export default function SkillDetailModal({ skill, copy, lang, onClose, onUse, t }) {
  ...
  return (
    <Modal onClose={onClose} ariaLabelledby="skill-detail-title" className="flex max-h-[85vh] max-w-xl flex-col overflow-hidden">
        ...
        <div className="flex justify-end border-t border-ink/15 px-5 py-4"><button type="button" onClick={onUse} disabled={skill.runnable === false} className="btn-primary">{skill.runnable === false ? t('skillsMarket.unavailable') : t('nav.useSkill')}</button></div>
    </Modal>
  )
}

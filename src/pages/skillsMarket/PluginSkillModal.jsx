...
export default function PluginSkillModal({ market, t }) {
  ...
  const onClose = () => market.setPluginState((current) => ({ ...current, open: false }))
  return (
    <Modal onClose={onClose} ariaLabelledby="plugin-skill-modal-title" className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-5 py-3 border-b border-ink/20"><div className="flex items-center gap-2"><Package className="w-4 h-4" /><h2 id="plugin-skill-modal-title" className="font-semibold text-base">{t('skillsMarket.pluginTitle')}</h2></div><button type="button" onClick={onClose} className="text-ink-fade hover:text-ink" aria-label={t('skillsMarket.close')}><X className="w-4 h-4" /></button></div>
        <div className="flex-1 overflow-auto p-5">
          {state.loading && <div className="text-sm text-ink-soft">{t('skillsMarket.loading')}</div>}
          {state.error && <div className="mb-3 p-2 border border-danger/35 bg-danger/5 rounded-md text-sm text-danger">{state.error}</div>}

...
export default function JobOverviewCard({
  ...
  return (
    <div className="rounded-md border border-ink/20 bg-paper-2 p-4">
      ...
      {job.error && <div className="mt-3 rounded-md border border-dashed border-danger/45 bg-danger/5 p-3"><p className="text-xs font-medium text-danger">{t('taskCenter.failureReason')}</p><p className="text-sm text-ink mt-1 break-words">{job.error}</p></div>}

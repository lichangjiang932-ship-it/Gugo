  ...
  return (
    <aside className="border-r border-dashed border-ink-fade/40 p-4 overflow-y-auto">
      ...
      {loading ? <p className="text-sm text-ink-fade">{t('taskCenter.loading')}</p> : visibleJobs.length === 0 ? <div className="rounded-md border border-dashed border-ink-fade/40 p-4 text-sm text-ink-fade">{t('taskCenter.empty')}</div> : (
        <div className="flex flex-col gap-2">{visibleJobs.map((job) => <button type="button" key={job.id} onClick={() => onSelect(job.id)} className={`text-left rounded-md border p-3 transition-colors ${selectedJobId === job.id ? 'border-ember bg-ember-soft' : 'border-ink/20 hover:border-ink/40'}`}><div className="flex items-center justify-between gap-2"><span className="text-sm text-ink line-clamp-2">{job.title}</span><span className="text-[10px] text-ink-fade shrink-0">{job.progress}%</span></div><div className="mt-2 flex items-center justify-between text-[11px] text-ink-soft"><span>{statusLabel(job.status)}</span><span>{formatTime(job.updatedAt)}</span></div></button>)}</div>
      )}
    </aside>
  )
}

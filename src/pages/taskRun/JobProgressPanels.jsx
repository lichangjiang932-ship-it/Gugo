...
export default function JobProgressPanels({ job, selectedArtifact, setSelectedArtifact, statusLabel, onRetryStep, t }) {
  return (
    <div className="grid grid-cols-[1.2fr_0.8fr] gap-4">
      <section className="rounded-md border border-ink/20 p-4"><div className="flex items-center gap-2 mb-3"><Activity className="w-4 h-4 text-ember" /><h3 className="font-semibold text-lg text-ink">{t('taskCenter.subtasks')}</h3></div><div className="flex flex-col gap-2">{(job.steps || []).map((step) => <StepCard key={step.id} step={step} statusLabel={statusLabel} onRetry={onRetryStep} t={t} />)}</div></section>
      <div className="flex flex-col gap-4">
        <section className="rounded-md border border-ink/20 p-4"><div className="flex items-center gap-2 mb-3"><Clock3 className="w-4 h-4 text-ember" /><h3 className="font-semibold text-lg text-ink">{t('taskCenter.events')}</h3></div><div className="flex flex-col gap-2 max-h-64 overflow-y-auto">{(job.events || []).slice().reverse().map((event) => <div key={event.id} className="text-xs"><p className="text-ink">{event.message}</p><p className="text-ink-fade">{formatTime(event.createdAt)}</p></div>)}</div></section>
        <section className="rounded-md border border-ink/20 p-4"><div className="flex items-center gap-2 mb-3"><CheckCircle2 className="w-4 h-4 text-ember" /><h3 className="font-semibold text-lg text-ink">{t('taskCenter.artifacts')}</h3></div>{(job.artifacts || []).length ? <div className="flex flex-col gap-2">{job.artifacts.map((artifact) => <ArtifactRow key={artifact.id} artifact={artifact} active={selectedArtifact?.id === artifact.id} onSelect={setSelectedArtifact} t={t} />)}</div> : <p className="text-xs text-ink-fade">{t('taskCenter.noArtifacts')}</p>}</section>
      </div>
    </div>
  )
}

  ...
  return (
    <header className="px-7 py-5 border-b border-dashed border-ink-fade/40">
      ...
      <form onSubmit={onCreate} className="mt-4 flex gap-2">
        ...
        <button type="submit" disabled={submitting || !prompt.trim()} className="h-11 px-5 rounded-md bg-accent text-accent-contrast font-semibold text-sm disabled:opacity-50">{submitting ? t('taskCenter.creating') : t('taskCenter.start')}</button>
      </form>

...
function EmptyChannel({ onCreate, t }) {
  return (
    <div className="flex flex-1 items-center justify-center p-6"><div className="flex w-full max-w-md flex-col items-center gap-4 rounded-md border border-ink-fade/40 bg-paper p-6 text-center"><div className="flex h-14 w-14 items-center justify-center rounded-md border border-ink-fade/40 bg-paper-2"><MessageSquarePlus className="h-7 w-7 text-ink-soft" /></div><div><h2 className="font-hand text-2xl text-ink">{t('channels.emptyTitle')}</h2><p className="mt-2 text-sm leading-relaxed text-ink-soft">{t('channels.emptyHint')}</p></div><div className="flex flex-col items-center gap-2"><button type="button" onClick={onCreate} className="h-10 rounded-md bg-ember px-4 text-sm text-paper hover:bg-ember/90">{t('channels.emptyCta')}</button><Link to="/settings" className="text-xs text-ink-fade hover:text-ink-soft">{t('channels.emptyBindIm')}</Link></div></div></div>
  )
}

...
export default function ComposerAttachments({ attachments, onClear, onOpen, onRemove, t }) {
  ...
  return <div className="mb-3 flex flex-wrap gap-2">
    {attachments.map((item) => <div key={item.id} className="flex max-w-[280px] flex-wrap items-center gap-2 rounded-md border border-ink-fade/40 bg-paper-2 px-2 py-1.5 text-xs text-ink-soft">
      ...
      {item.uploadStatus === 'uploading' && <p className="w-full text-[10px] leading-4 text-ink-fade">{t('chatAttachments.uploading')}</p>}
      {item.uploadStatus === 'error' && <p className="w-full text-[10px] leading-4 text-rose-700">{t('chatAttachments.uploadFailed')}{item.uploadError ? `：${item.uploadError}` : ''}</p>}
    </div>)}

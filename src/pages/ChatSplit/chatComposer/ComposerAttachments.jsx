...
export default function ComposerAttachments({ attachments, onClear, onOpen, onRemove, t }) {
  ...
  return <div className="mb-3 flex flex-wrap gap-2">
    {attachments.map((item) => {
      const canOpen = typeof onOpen === 'function' && Boolean(item.downloadUrl || item.dataUrl)
      const openTitle = t('chatMessages.openPreviewTitle')
      const icon = item.kind === 'image' && item.dataUrl
        ? <img src={item.dataUrl} alt="" className="h-7 w-7 shrink-0 rounded border border-ink-fade/30 object-cover" />
        : <FileText className="h-4 w-4 shrink-0 text-ink-fade" />
      const label = <span className="min-w-0 flex-1 truncate">{item.name} · {item.sizeKB}KB</span>
      return <div key={item.id} className="flex max-w-[280px] flex-wrap items-center gap-2 rounded-control border border-ink-fade/40 bg-paper-2 px-2 py-1.5 text-xs text-ink-soft">
        {canOpen ? (
          <button
            type="button"

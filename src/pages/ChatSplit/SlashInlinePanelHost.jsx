  ...
  return (
    <PanelShell testId="slash-mcp-panel" icon={Plug} title={copy.title} closeLabel={copy.close} onClose={onClose}>
      ...
      {!state.loading && state.error && (
        <div className="mt-3 flex items-center gap-3 rounded-card bg-red-500/[0.06] px-3 py-2.5 text-xs text-red-700">
          <span className="min-w-0 flex-1 truncate" title={state.error}>{copy.loadError}: {state.error}</span>
          <button type="button" onClick={load} className="rounded-control px-2 py-1 hover:bg-red-500/10">{copy.retry}</button>
        </div>
      )}
      {!state.loading && !state.error && (
        <>
              ...
              {state.servers.map((server) => {
                ...
                return (
                  <div key={server.id} className="flex items-center gap-3 rounded-card px-3 py-2 hover:bg-ink/[0.035]">
                    <span className={`h-2 w-2 shrink-0 rounded-pill ${connected ? 'bg-emerald-500' : 'bg-ink/20'}`} />

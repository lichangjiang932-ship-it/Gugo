...
export function ServerArtifactCards({ artifacts = [] }) {
  ...
  return <div className="mt-3 space-y-2" data-testid="server-turn-artifacts" data-artifact-surface="server-artifacts">{artifacts.map((artifact) => <a key={artifact.id || artifact.url} href={withDownloadToken(artifact.url)} download={artifact.filename || ''} className="flex items-center gap-3 rounded-md border border-ink/10 bg-paper p-3 transition-colors hover:border-ink/20 hover:bg-paper-2/45"><span className="flex h-9 w-9 items-center justify-center rounded-md bg-ink/[0.055] text-ink-fade"><FileText className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="chat-output-file-name block truncate text-sm font-medium text-ink">{artifact.filename || artifact.title || 'artifact'}</span><span className="block text-xs text-ink-fade">{artifact.type || 'file'}</span></span><Download className="h-4 w-4 text-ink-fade" /></a>)}</div>
}

export function ArtifactOpenCard({ preview, onOpen, className = '' }) {
  const { t } = useT()
  if (!preview) return null
  return <button type="button" data-testid="artifact-open-card" data-artifact-surface="artifact-card" onClick={onOpen} className={`group flex w-full items-center gap-3 rounded-md border border-ink/10 bg-paper p-3 text-left transition-colors hover:border-ink/20 hover:bg-paper-2/45 ${className}`} title={`${preview.label} · ${t('chatPreview.preview')}`}><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-ink/10 bg-ink/[0.045] text-ink-fade"><PreviewIcon type={preview.type} /></div><div className="min-w-0 flex-1"><div className="font-mono text-[10px] uppercase tracking-[0.18em] text-ink-fade">{preview.label}</div><div className="chat-output-file-name truncate text-sm font-semibold text-ink" title={preview.filename}>{preview.filename}</div><div className="truncate text-xs text-ink-fade">{preview.summary}</div></div><div className="inline-flex shrink-0 items-center gap-1.5"><span className="hidden text-[11px] text-ink-fade group-hover:text-ink sm:inline">{t('chatPreview.preview')}</span><ExternalLink className="h-4 w-4 text-ink-fade group-hover:text-ink" /></div></button>
}

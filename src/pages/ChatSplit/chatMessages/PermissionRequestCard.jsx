    ...
    <AnimatePresence>
      {request && (
        <motion.div
          ...
          className="ml-10 rounded-card border border-ember bg-ember-soft p-4 shadow-sm animate-pulse-ember"
        >
          <div className="mb-3 flex items-center justify-between">
            <span className="font-mono text-xs tracking-wider text-ember">{t('chatMessages.permissionRequest', { name: request.skillName })}</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={onAllow} className="h-9 rounded-control bg-ember px-4 font-semibold text-sm text-paper transition-colors hover:bg-ember/90">{t('chatMessages.allowContinue')}</button>

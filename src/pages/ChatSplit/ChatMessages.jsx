    ...
    <div className="chat-messages-shell relative min-h-0 flex-1">
      <div ref={bindContainer} className="chat-scroll-region relative h-full overflow-y-auto px-4 py-4 sm:px-7">
        ...
        {!atBottom && messages.length > 0 && (
          <button onClick={scrollToBottom} className="absolute bottom-4 right-6 z-10 inline-flex h-8 items-center gap-1.5 rounded-full border border-ink-fade/45 bg-paper px-3 text-xs text-ink-soft transition-colors hover:border-ink-fade hover:text-ink" title={t('chatMessages.backToBottom')} aria-label={t('chatMessages.backToBottom')}>
        ...
        {quoteBubble && (
          <button
            ...
            className="absolute z-20 inline-flex h-7 items-center gap-1 rounded-full bg-ink px-2.5 text-xs font-medium text-paper transition-colors hover:bg-ember"

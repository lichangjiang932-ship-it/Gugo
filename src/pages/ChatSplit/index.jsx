...
export default function ChatSplit() {
  ...
  // Use ref to always access latest state inside async callbacks
  ...
  const triggerSendFlow = useCallback(
    async (content, explicitAttachments = null) => {
      ...
      const activeSession = latestState.sessions.find((s) => s.id === activeSessionId)
      ...
      if (isFreshSession) {
        ...
        const sessionIdSnapshot = latestState.activeSessionId
      ...
      // 登录检查:未登录仍然继续尝试,API调用失败时会显示友好提示
      // 这样静态部署时用户也能看到完整UI和交互
      ...
      try {
        const messages = []
        const systemPrompt = skillId ? getSkillSystemPrompt(skillId, latestState.skillConfigs, runtimeSkills) : ''
        ...
        if (agentMode === 'plan') {
          ...
          messages.push({
            role: 'system',
            content: 'Research mode: you are a systematic research analyst. When the user asks a question, use web_search to find the latest information, then fetch_url to read key sources deeply. Always cite sources with URLs. Synthesize findings into a structured report with: key findings, supporting evidence, conflicting viewpoints, and confidence levels. Use create_mermaid for concept maps when helpful. Prefer depth over breadth — 3 well-analyzed sources beat 10 shallow ones.',
          })
        } else if (agentMode === 'write') {
          messages.push({
            role: 'system',
            content: 'Write mode: you are a professional content creator. Help users produce polished documents, presentations, and structured content. Use create_docx for reports, create_pptx for presentations, create_mermaid for diagrams, and create_xlsx for data tables. Focus on clarity, structure, and visual appeal. Ask clarifying questions about audience/purpose if unclear. Always produce actionable, well-formatted output.',
          })
        } else if (agentMode === 'debug') {
          messages.push({
            role: 'system',
            content: 'Debug mode: you are a senior debugging engineer. Methodically diagnose issues: (1) read relevant files, (2) analyze error patterns, (3) check git diff for recent changes, (4) run lint/test/build to reproduce, (5) propose minimal targeted fixes. Explain root cause before proposing fixes. Use bash_exec cautiously — only read/validate, never destructive operations. Prefer one-line or small-scope fixes.',
          })
        }
        ...
        try {
          ...
          const enabledToolNames = resolveToolsForMode(latestState.toolsConfig || {}, agentMode)
        ...
        const notifyPerm = latestState.permissions.find((p) => p.id === 'notify')
        ...
        dispatch({ type: 'APPEND_TO_LAST_MESSAGE', payload: isOffline
          ? `\n\n无法连接到后端服务。当前为前端演示模式，所有界面功能正常，但 AI 回复需要配置后端 API。\n\n**本地开发**：运行 \\"npm run dev\\" 启动后端服务\n**部署环境**：配置 MODEL_BASE_URL、MODEL_NAME、MODEL_API_KEY`
          : `\n\n模型调用失败：${err.message}`
        })

...
export default function MemoryView() {
  ...
  return <div className="flex h-screen overflow-hidden bg-paper"><LeftRail /><div className="flex min-w-0 flex-1 flex-col">
    <div className="flex items-center gap-3 border-b border-ink/10 px-6 py-4"><BookOpen className="h-5 w-5 text-ember" /><div className="flex-1"><div className="text-base font-semibold text-ink">{t('memory.title')}</div><div className="text-[11px] text-ink-fade">{t('memory.subtitle')}</div></div><button type="button" onClick={handleNew} className="flex h-8 items-center gap-1 rounded-md bg-ember px-3 text-xs text-paper hover:bg-ember/90"><Plus className="h-3.5 w-3.5" />{t('memory.add')}</button></div>

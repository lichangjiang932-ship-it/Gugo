...
export default function HookEditor({ editing, onChange, onClose, onDelete, onSave, onTest, saving, testResult, t }) {
  ...
  return <div className="mx-auto max-w-[720px] space-y-4 px-8 py-6">
    ...
    <Field label={t('hooks.toolPattern')}><input value={editing.toolPattern} onChange={(event) => update({ toolPattern: event.target.value })} className="h-9 w-full rounded-md border border-ink/15 bg-paper-2 px-3 text-sm outline-none focus:border-ember" /></Field>
    <Field label={t('hooks.argumentMatcher')}>
      <textarea rows="4" value={editing.argumentMatcherText || ''} onChange={(event) => update({ argumentMatcherText: event.target.value })} placeholder={t('hooks.argumentMatcherPlaceholder')} spellCheck="false" className="w-full rounded-md border border-ink/15 bg-paper-2 px-3 py-2 font-mono text-xs outline-none focus:border-ember" />
    ...
    {editing.kind === 'http' ? <Field label="HTTPS URL (POST JSON)"><input value={editing.url || ''} onChange={(event) => update({ url: event.target.value })} placeholder="https://your-host/hook" className="h-9 w-full rounded-md border border-ink/15 bg-paper-2 px-3 text-sm outline-none focus:border-ember" /></Field> : <Field label={t('hooks.shellArgv')}><input value={Array.isArray(editing.command) ? editing.command.join(' ') : editing.command || ''} onChange={(event) => update({ command: event.target.value })} placeholder="node hooks/audit.js" className="h-9 w-full rounded-md border border-ink/15 bg-paper-2 px-3 font-mono text-sm outline-none focus:border-ember" /></Field>}
    ...
    <div className="flex items-center gap-2 pt-2"><button type="button" onClick={onSave} disabled={saving} className="flex h-8 items-center gap-1 rounded-md bg-ember px-4 text-xs text-paper hover:bg-ember/90 disabled:opacity-50"><Save className="h-3.5 w-3.5" />{saving ? t('hooks.saving') : t('hooks.save')}</button>{editing.id && <><button type="button" onClick={() => onTest(editing.id)} className="flex h-8 items-center gap-1 rounded-md border border-ink/15 px-3 text-xs text-ink-soft hover:bg-paper-2"><Play className="h-3.5 w-3.5" />{t('hooks.test')}</button><button type="button" onClick={() => onDelete(editing.id)} className="flex h-8 items-center gap-1 rounded-md border border-rose-300 px-3 text-xs text-rose-700 hover:bg-rose-50"><Trash2 className="h-3.5 w-3.5" />{t('hooks.delete')}</button></>}</div>
...
function Field({ label, children }) { return <div><label className="mb-1.5 block text-[11px] text-ink-fade">{label}</label>{children}</div> }
function KindChip({ active, onClick, children }) { return <button type="button" onClick={onClick} className={`rounded-md border px-3 py-1 text-xs transition-colors ${active ? 'border-ember bg-ember text-paper' : 'border-ink/15 bg-paper-2 text-ink-soft hover:border-ember/50'}`}>{children}</button> }

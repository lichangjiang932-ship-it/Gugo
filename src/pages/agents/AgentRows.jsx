        ...
        <li key={agent.id} className="-mx-2 flex items-center justify-between rounded px-2 py-4 hover:bg-ink/5">
          <button onClick={() => onEdit(agent)} className="flex-1 text-left">
            <div className="flex items-center gap-2">
              ...
              {agent.isDefault && <span className="inline-flex items-center gap-1 rounded bg-amber-100/50 px-2 py-0.5 text-xs text-amber-700"><Star size={10} />{t('agents.defaultBadge')}</span>}
              ...
              {(agent.personaManifest?.capabilityIds || []).slice(0, 2).map((id) => <span key={id} className="inline-flex rounded bg-blue-50 px-2 py-0.5 text-xs text-blue-700">{id}</span>)}
            </div>
...
function IconButton({ children, danger = false, label, onClick, title }) {
  return <button onClick={onClick} className={`ml-2 p-2 text-ink-fade ${danger ? 'hover:text-red-600' : 'hover:text-ink'}`} aria-label={label} title={title}>{children}</button>
}

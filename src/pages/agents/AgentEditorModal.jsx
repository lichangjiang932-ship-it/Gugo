...
export default function AgentEditorModal({
  ...
  return (
    <Modal onClose={onClose} ariaLabelledby="agent-editor-title" overlayClassName="p-6" className="flex max-h-[88vh] max-w-3xl flex-col overflow-hidden">
        <header className="flex items-center justify-between border-b border-ink/10 px-6 py-4">
          <h2 id="agent-editor-title" className="text-lg font-semibold">{agent.id ? t('agents.editTitle') : t('agents.newTitle')}</h2>
          <button onClick={onClose} className="text-ink-fade hover:text-ink"><X size={18} /></button>
        </header>
        <div className="flex-1 space-y-5 overflow-y-auto px-6 py-5">
          <Field label={t('agents.fieldName')}>
            <input value={agent.name} onChange={(event) => onChange({ ...agent, name: event.target.value })} className="w-full rounded border border-ink/15 bg-paper px-3 py-2 text-sm text-ink" placeholder={t('agents.namePlaceholder')} maxLength={80} />
          </Field>
          <div className="space-y-3 rounded-md border border-ink/10 p-4">
            ...
            <div className="flex gap-2">
              <select value={personaDraftId} onChange={(event) => onPersonaSelect(event.target.value)} className="flex-1 rounded border border-ink/15 bg-paper px-3 py-2 text-sm text-ink">
          ...
          <Field label={t('agents.fieldSoul')}>
            <textarea value={agent.soulMd} onChange={(event) => onChange({ ...agent, soulMd: event.target.value })} rows={10} className="w-full rounded border border-ink/15 bg-paper px-3 py-2 font-mono text-sm text-ink" placeholder={t('agents.soulPlaceholder')} />
          </Field>
          <Field label={t('agents.fieldIdentity')}>
            <textarea value={agent.identityMd} onChange={(event) => onChange({ ...agent, identityMd: event.target.value })} rows={6} className="w-full rounded border border-ink/15 bg-paper px-3 py-2 font-mono text-sm text-ink" placeholder={t('agents.identityPlaceholder')} />
          </Field>
          <Field label={t('agents.fieldAvatar')}>
            <input value={agent.avatarUrl} onChange={(event) => onChange({ ...agent, avatarUrl: event.target.value })} className="w-full rounded border border-ink/15 bg-paper px-3 py-2 text-sm text-ink" placeholder="https://..." maxLength={1024} />
          </Field>
        ...
        <footer className="flex items-center justify-end gap-2 border-t border-ink/10 px-6 py-4">
          ...
          <button onClick={onSave} disabled={saving} className="inline-flex items-center gap-2 rounded bg-ink px-4 py-2 text-sm text-paper disabled:opacity-50"><Save size={14} />{saving ? t('common.saving') : t('common.save')}</button>
        </footer>
    </Modal>
  )
}

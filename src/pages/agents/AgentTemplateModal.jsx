...
export default function AgentTemplateModal({ loading, onClose, onPreview, onUse, preview, source, templates, t }) {
  return (
    <Modal onClose={onClose} ariaLabelledby="agent-template-title" overlayClassName="bg-ink/30" className="flex max-h-[85vh] max-w-4xl flex-col overflow-hidden">
        <header className="flex items-center justify-between border-b border-ink/10 px-6 py-4">
          <h2 id="agent-template-title" className="text-base font-medium">{t('agents.templatesTitle')}</h2>
        ...
        {preview && (
          <footer className="flex items-center justify-end gap-2 border-t border-ink/10 px-6 py-3">
            ...
            <button onClick={() => onUse(preview)} className="inline-flex items-center gap-2 rounded bg-ink px-4 py-2 text-sm text-paper hover:opacity-90"><Sparkles size={14} />{t('agents.useThis')}</button>
          </footer>
        )}
    </Modal>
  )

...
export const translations = {
  ...
  ja: {
    ...
    errors: {
      ...
      modelProviderUnavailable: 'このタスクに紐づくモデル Provider は現在利用できません。URL、認証情報、モデル名を確認してください。',
      modelProviderAmbiguous: '複数の Provider が同じモデル名を提供しています。このタスクで使用する Provider を明示的に選択してください。',
      modelProviderBindingMissing: 'タスクに検証可能なモデル Provider の紐付けがありません。モデルを確認してタスクを再作成してください。',
      modelProviderConfigChanged: 'タスク作成後に紐付け済みのモデル Provider 設定が変更されました。設定を確認してタスクを再作成してください。',
      modelRequestOutcomeUnknown: 'モデル要求の最終結果を確認できません。重複実行を防ぐため、続行前に Provider の記録を確認してください。',
      ...
      modelOutboundBlocked: 'モデルエンドポイントはローカル送信セキュリティポリシーにより拒否されました。URL、DNS、リダイレクト先を確認してください。',
      modelContextLimit: 'タスクがモデルのコンテキスト長を超えています。入力を短くするか、より大きなコンテキストのモデルを選択してください。',
      modelRequestRejected: 'モデルサービスがこの要求を拒否しました。モデル名と互換設定を確認してください。',
      modelUpstreamError: '上流のモデルサービスがエラーを返しました。サービス状態を確認して再試行してください。',
  ...
  ko: {
    ...
    errors: {
      ...
      modelProviderUnavailable: '이 작업에 연결된 모델 Provider를 현재 사용할 수 없습니다. URL, 인증 정보, 모델 이름을 확인하세요.',
      modelProviderAmbiguous: '여러 Provider가 같은 모델 이름을 제공합니다. 이 작업에서 사용할 Provider를 명시적으로 선택하세요.',
      modelProviderBindingMissing: '작업에 검증 가능한 모델 Provider 연결 정보가 없습니다. 모델을 확인한 뒤 작업을 다시 만드세요.',
      modelProviderConfigChanged: '작업을 만든 후 연결된 모델 Provider 설정이 변경되었습니다. 설정을 확인한 뒤 작업을 다시 만드세요.',
      modelRequestOutcomeUnknown: '모델 요청의 최종 결과를 확인할 수 없습니다. 중복 실행을 막으려면 계속하기 전에 Provider 기록을 확인하세요.',
      ...
      modelOutboundBlocked: '모델 엔드포인트가 로컬 아웃바운드 보안 정책에 의해 거부되었습니다. URL, DNS, 리디렉션 대상을 확인하세요.',
      modelContextLimit: '작업이 모델의 컨텍스트 길이를 초과합니다. 입력을 줄이거나 더 큰 컨텍스트를 지원하는 모델을 선택하세요.',
      modelRequestRejected: '모델 서비스가 이 요청을 거부했습니다. 모델 이름과 호환성 설정을 확인하세요.',
      modelUpstreamError: '상위 모델 서비스가 오류를 반환했습니다. 서비스 상태를 확인한 뒤 다시 시도하세요.',
  ...
  'zh-TW': {
    ...
    errors: {
      ...
      modelProviderUnavailable: '任務綁定的模型 Provider 目前無法使用，請檢查位址、憑證和模型名稱。',
      modelProviderAmbiguous: '多個 Provider 提供同名模型，請明確選擇本任務要使用的 Provider。',
      modelProviderBindingMissing: '任務缺少可驗證的模型 Provider 綁定，請確認模型後重新建立任務。',
      modelProviderConfigChanged: '任務建立後模型 Provider 設定已變更，請確認目前設定後重新建立任務。',
      modelRequestOutcomeUnknown: '模型請求的最終結果無法確認。為避免重複執行，請先核對 Provider 記錄再繼續。',
      ...
      modelOutboundBlocked: '模型端點被本機出站安全策略拒絕，請檢查 URL、DNS 和重導向目標。',
      modelContextLimit: '任務內容超出模型上下文長度，請縮短輸入或選擇更大上下文的模型。',
      modelRequestRejected: '模型服務拒絕了本次請求，請檢查模型名稱和相容設定。',
      modelUpstreamError: '模型上游服務回傳錯誤，請檢查服務狀態後重試。',

...
Object.assign(FEATURE_PAGE_TRANSLATIONS['zh-TW'].chatMessages, {
  ...
  incompleteRequirementDirectory: '可繼續使用的授權目錄', incompleteRequirementBoundedModelResponse: '一則可在安全推理上限內完成的模型回應', incompleteRequirementArtifact: '符合目標且通過驗證的最終檔案', incompleteRequirementSelection: '明確的最終交付檔案選擇', incompleteRequirementProjectCheck: '差異檢查或專案檢查結果', incompleteRequirementExecutionEvidence: '與修改目標對應的實際執行證據', incompleteRequirementHtmlValidation: 'HTML 本機資源完整性驗證', incompleteRequirementModelResponse: '一則可顯示且可作為終答的模型回應', incompleteRequirementReadback: '修改後檔案的讀回驗證', incompleteRequirementPdfValidation: '目標頁、非目標頁、文字邊界與逐頁渲染驗證', incompleteRequirementProgress: '從最近檢查點繼續後取得的新進展', incompleteRequirementRemainingSteps: '尚未執行或尚未驗證的任務步驟',
})

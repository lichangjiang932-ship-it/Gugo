...
export const FS_SHELL_TOOL_SPECS = [
  ...
  },
  {
    ...
    function: {
      name: 'read_file',
      description: '只读检查工作区或用户已授权本地文件：UTF-8 返回全文或行区间；PDF 提取可选文本；DOCX/PPTX/XLSX 提取文本，并对本地 Office 文件返回 formatValidated 格式校验结果。检查 PPTX 请直接读取最终 .pptx 路径，不要用 pdf_info/pdf_text。formatValidated=true 只证明结构校验通过，不证明视觉布局。额外授权范围请使用绝对路径，本地文件超过 5MB 会拒绝；不写入或修改文件。',
      parameters: {
        type: 'object',
        properties: {
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          expected_outputs: { type: 'array', default: [], items: { type: 'string' }, description: '仅声明本次命令实际创建或修改的文件；只读检查/验证命令省略或传 []，不要列入被检查但未改变的输入文件。' },

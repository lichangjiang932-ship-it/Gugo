...
async function handleOAuthCallback(req, res, url, provider, { env, fetchImpl }) {
  try {
...
function handleVisionAssistStatus(res, userId) {
...
async function handleIntegrationCrud(req, res, url, parts, userId, env) {
  try {
    ...
    if (req.method === 'GET' && parts[2] === 'oauth' && parts[3] === 'sessions' && parts[4]) {
      const session = getOAuthConnectionStatus({ userId, id: decodeURIComponent(parts[4]) })
...
export async function handleIntegrationsRequest(req, res, { env = process.env, fetchImpl = fetch } = {}) {
  ...
  if (req.method === 'GET' && parts[2] === 'oauth' && parts[3] === 'callback' && parts[4]) {
    return handleOAuthCallback(req, res, url, decodeURIComponent(parts[4]), { env, fetchImpl })
  }
  const userId = authenticateRequest(req)
  if (!userId) return unauthorized(res)
  if (req.method === 'GET' && parts[2] === 'vision_assist' && parts[3] === 'status') {
    return handleVisionAssistStatus(res, userId)
  }
  return handleIntegrationCrud(req, res, url, parts, userId, env)
}

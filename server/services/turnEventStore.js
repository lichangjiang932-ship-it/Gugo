...
import { canAdvanceTurnEventCursor, createTurnEventTransportEnvelope, parsePersistedTurnEvent, parseTurnEvent } from '../../shared/turnEvents.js'
import {
...
export {
  ...
  acknowledgeTurnEventWriteFailuresByEventIds,
  DEFAULT_EVENT_WRITE_FAILURE_MAX_PER_USER,
  DEFAULT_EVENT_WRITE_FAILURE_RETENTION_DAYS,
  listTurnEventWriteFailures,
  pruneTurnEventWriteFailures,
  recordTurnEventWriteFailure,
} from './turnEventWriteFailureStore.js'
...
export function appendTurnEventsInTransaction(entries = [], db, {
  ...
  `)
  const insertEvent = db.prepare(`INSERT INTO turn_events
    (id, user_id, session_id, turn_id, sequence, type, payload_json, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO NOTHING ON CONFLICT(user_id, session_id, turn_id, sequence) DO NOTHING`)
...
export function replayTurnEventWriteFailure({ userId, id } = {}) {
  return replayStoredTurnEventWriteFailure({ userId, id }, appendTurnEvent)
}

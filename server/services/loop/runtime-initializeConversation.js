...
function initializeConversationContext(s) {
  ...
  } catch { /* prompt context is best-effort */ }
  const authorizedRoots = s.directoryAuthorizationResolutions
    .filter((resolution) => (
  ...
  const configuredRoot = path.isAbsolute(String(s.outputDirectoryContext.projectDirectory || '').trim())
...
function initializeArtifactProvenance(s) {
  const { normalizeArtifactIdList } = s.d
    ...
    (Array.isArray(s.restoredState?.completionGuards?.artifactProvenance)
      ...
      .map((entry) => [
        String(entry?.artifactId || '').trim(),
        {
          ...
          ...(entry?.artifactType
            ? { artifactType: String(entry.artifactType).trim().toLowerCase() }
            : {}),
...
function installTerminalProtection(s) {
  const { sanitizeIncompleteTerminalText, sourceHandoffViolation, terminalProtectionCopy } = s.d
  ...
  s.protectTerminalText = (text, { incomplete = false } = {}) => {
    ...
    if (s.artifactIds.length > 0) {
      return incomplete ? terminalCopy.unverifiedFileText : terminalCopy.filteredCompletedFileText
    }
    return incomplete ? terminalCopy.incompleteTaskText : terminalCopy.filteredCompletedTaskText
  }
  s.protectClarification = (clarification) => {
    if (!clarification || typeof clarification !== 'object') return clarification
    ...
    const protectValue = (value) => {
      ...
      return Object.fromEntries(
        Object.entries(value).map(([key, nested]) => [key, protectValue(nested)]),
      )
    }
...
function initializeDeliverableSelection(s) {
  const { normalizeArtifactIdList, sameArtifactIdList } = s.d
  ...
  s.deliveryArtifactIds = s.deliveryArtifactSelectionExplicit
    ...
    : []
  s.restoredSelectionArtifactIds = s.restoredState?.completionGuards
...
function installArtifactRecording(s) {
  const { isFileArtifactTool, listTurnArtifacts, normalizeArtifactIdList } = s.d
...
export async function initializeConversation(s) {
  initializeConversationContext(s)
  initializeArtifactProvenance(s)
  installTerminalProtection(s)
  initializeDeliverableSelection(s)
  installArtifactRecording(s)
  return { kind: 'next' }
}

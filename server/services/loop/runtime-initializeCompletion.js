...
function initializeArtifactCompletionState(s) {
  const {
  ...
  s.artifactContractToolsForProvenance = (provenance) => {
    ...
    const declaredOutput = isCommandExecutionTool(sourceTool)
    ...
    if (!declaredOutput || !artifactType) return covered
  ...
  s.recomputeDeliveredArtifactTools = () => {
    ...
    for (const [artifactId, provenance] of s.artifactProvenance) {
      if (!s.artifactIds.includes(artifactId) || provenance?.verified !== true) continue
  ...
  s.inheritedArtifactEvidence = ['verify', 'finalize'].includes(String(s.step?.kind || ''))
    ...
    && s.job.steps.some((step) => step?.id !== s.step?.id
      && step?.status === 'completed'
      && Array.isArray(step?.output?.artifactIds)
      && step.output.artifactIds.length > 0)
...
function initializeMutationVerification(s) {
  const {
  ...
  s.guardPriorOutcomeStatusText = (value) => {
    ...
    const verifiedFiles = Array.isArray(s.priorTurnOutcome?.verifiedLocalFiles)
      ? s.priorTurnOutcome.verifiedLocalFiles
          .map((file) => String(file?.path || '').trim()).filter(Boolean)
      : []
  ...
  const rawTargets = Array.isArray(s.restoredState?.completionGuards?.pendingMutationTargets)
    ? s.restoredState.completionGuards.pendingMutationTargets
    : s.restoredState?.completionGuards?.pendingMutationVerification ? [PROJECT_SCOPE_TARGET] : []
...
function installLocalHtmlVerification(s) {
  const { LOCAL_HTML_DELIVERY_GUARD_MARKER, PROJECT_SCOPE_TARGET,
    getProjectDirectory, normalizeMutationTarget, path, targetsMatch,
    validateLocalHtmlDelivery } = s.d
...
function installTaskVerification(s) {
  const {
  ...
  s.hasPendingTaskVerificationRepair = () => hasPendingTaskVerificationRepair(s.taskVerificationRepair)
  s.taskVerificationRepairExhausted = () => taskVerificationRepairExhausted(s.taskVerificationRepair)
  s.taskVerificationRepairPrompt = () => buildTaskVerificationRepairPrompt(s.taskVerificationRepair)
  s.taskVerificationRepairBlockerText = () => taskVerificationRepairBlockerText(
    s.taskVerificationRepair, { locale: s.locale },
  )
  s.taskVerificationRepairDetails = () => taskVerificationRepairDetails(s.taskVerificationRepair)
  const workspaceRoot = s.verificationProjectDirectory
    || s.outputDirectoryContext?.projectDirectory || ''
  s.observeTaskVerificationMutation = (targets) => observeTaskVerificationMutation(
    s.taskVerificationRepair, targets, { workspaceRoot },
  )
  ...
  s.hasPendingMutationVerification = () => s.pendingMutationTargets.size > 0
...
function installCompletionPrompts(s) {
  ...
  if (s.priorArtifacts.length > 0 && s.revisesAdjacentArtifact
    ...
    const instruction = s.artifactRevisionMode === 'replace_original'
      ...
      : s.artifactRevisionMode === 'create_copy'
        ? ['The user explicitly requested a new or separate file and wants the original preserved.', 'Call the matching artifact generator without replace_artifact_id and deliver the newly returned artifact ID.', 'Do not overwrite or mutate the adjacent original artifact.']
  ...
  if ((s.directExecutionRequested || s.requiresPersistedArtifact)
    ...
    const notes = []
    if (commandNames.length) notes.push(`${commandNames.join('/')} can run commands and installed Python/Node scripts in an authorized workspace or local directory`)
    if (process.platform === 'win32' && commandNames.length && activeNames.includes('write_file')) notes.push(`on Windows, ${commandNames.join('/')} uses cmd.exe; for multiline or long Python such as PDF/image generation, write a UTF-8 .py file with write_file and then run that file instead of embedding the program in python -c, and do not use Unix-only tail/grep/sed/awk pipelines`)
    const writable = activeNames.filter((name) => FILE_WRITE_TOOL_NAMES.has(name))
    if (writable.length) notes.push(`${writable.join('/')} can create or modify authorized files`)
    const artifacts = activeNames.filter(isFileArtifactTool)
    if (artifacts.length) notes.push(`${artifacts.join('/')} can create persisted downloadable artifacts`)
    s.convo.push({ role: 'system', content: [
      AVAILABLE_TOOL_CAPABILITIES_MARKER,
      `The callable tools for this turn are: ${activeNames.join(', ') || '(none)'}.`,
...
function installFinalAnswerEvidence(s) {
  const {
  ...
  s.safeFallbackDeliverableIds = () => {
    ...
    const candidates = s.artifactIds.filter((id) => {
    ...
    for (const id of candidates) {
      for (const toolName of s.artifactContractToolsForProvenance(s.artifactProvenance.get(id))) {
        coveredTools.add(toolName)
      }
    }
    return [...s.expectedArtifactTools].every((name) => coveredTools.has(name)) ? candidates : []
  }
  s.applySafeDeliverableFallback = () => {
    if (!s.deliveryContractReadyForSelection()) return null
    const ids = s.safeFallbackDeliverableIds()
    if (ids.length === 0) return null
    const selection = s.selectDeliverables({ artifact_ids: ids })
    return selection?.ok === true
      ? { ...selection, code: 'deliverable_selection_safe_fallback', fallback: true }
      : null
  }
  s.missingArtifactBlocker = () => missingArtifactBlocker(s.missingArtifactTools())
  const restoredReview = s.restoredState?.completionGuards?.finalAnswerEvidenceReview
  const restoredEvidence = s.restoredState?.completionGuards?.finalAnswerToolEvidence
  s.finalAnswerToolEvidence = Array.isArray(restoredEvidence)
    ? normalizeFinalAnswerToolEvidence(restoredEvidence)
    : collectFinalAnswerToolEvidence(s.convo)
  s.finalAnswerEvidenceReview = /^[a-f0-9]{64}$/i.test(String(restoredReview?.digest || ''))
    ? { digest: String(restoredReview.digest), iteration: Math.max(0, Number(restoredReview.iteration) || 0) }
    : null
  ...
  s.finalAnswerEvidenceSnapshot = () => buildFinalAnswerEvidenceSnapshot({
    ...
    artifacts: s.artifactIds.map((id) => {
      const provenance = s.artifactProvenance.get(id)
      return { id, tool: provenance?.toolName, type: provenance?.artifactType, verified: provenance?.verified === true }
    }),
    ...
    postMutationVerificationPassed: s.mutationExecutionObserved
      && !s.hasPendingMutationVerification() && !s.hasPendingTaskVerificationRepair(),
  ...
  s.currentFinalAnswerEvidenceDigest = () => finalAnswerEvidenceDigest(s.finalAnswerEvidenceSnapshot())
  ...
  s.hasCurrentFinalAnswerEvidenceReview = (digest = null) => {
    if (!s.finalAnswerEvidenceReady()) return false
    const current = s.currentFinalAnswerEvidenceDigest()
    return s.finalAnswerEvidenceReview?.digest === current && (digest === null || digest === current)
  }
  s.prepareFinalAnswerEvidenceReview = () => {
    ...
    s.convo.push({ role: 'system', content: buildFinalAnswerEvidenceReviewPrompt(snapshot, digest) })
    return true
  }
}

export async function initializeCompletion(s) {
  initializeArtifactCompletionState(s)
  initializeMutationVerification(s)
  installLocalHtmlVerification(s)
  installTaskVerification(s)
  installCompletionPrompts(s)
  installFinalAnswerEvidence(s)
  return { kind: 'next' }
}

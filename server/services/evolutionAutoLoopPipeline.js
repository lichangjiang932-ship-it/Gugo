...
import {
  activateAutoCanary,
  runAutoReplayValidation,
} from './evolutionAutoRunStages.js'
...
export async function runQueuedEvolutionAutoRun({
  ...
  try {
    const evaluation = await runAutoReplayValidation({
      services,
      owner,
      row,
      config,
      ...
      signal,
      startedAt,
      supportedTarget: SUPPORTED_TARGET,
      parseJson,
      createReplayCases: replayCases,
      updateStage: updateRunningStage,
    })
    ...
    await activateAutoCanary({
      ...
      env,
      parseJson,
      defaultRollbackPolicy: DEFAULT_ROLLBACK_POLICY,
      updateStage: updateRunningStage,
    })
  } catch (error) {

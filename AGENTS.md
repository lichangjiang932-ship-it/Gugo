...
- 控制消息 marker 必须登记在 `server/services/loop/controlMarkers.js`，注明定义责任与用途。计数基线是治理门禁，禁止仅改数字消除失败；新增/删除应同时审查真实 issuer/consumer、信任与时序边界、恢复路径及 `controlMarkerRegistry`/`loopMarkers` 行为回归，并在变更记录说明理由。
